  ...
  return (
    <section className="mb-16">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-3xl font-bold text-text-primary mb-2">Trending Topics</h2>
          <p className="text-text-secondary">Hot topics in embedded systems development</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {topics?.map((topic, index) => (
          <Link
            key={topic?.id}
            to={`/industry-insights-hub/topic/${topic?.slug}`}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-1">
                    ...
                    <span className="text-text-muted text-xs">{topic?.articleCount} articles</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <Icon name="Eye" size={14} className="text-text-muted" />
                    <span className="text-text-muted text-xs">{topic?.views}</span>
                  </div>
                </div>
